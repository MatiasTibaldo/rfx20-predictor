"""
Entry point para Track B, Etapa 2 (Bloque 2 del plan de acción): GRU por
horizonte sobre el índice RFX20.

Mismo protocolo que models/deep_learning/lstm_runner.py (windowing,
escalado, split de early-stopping, un modelo por horizonte) — solo cambia
la celda recurrente, para aislar el efecto de LSTM vs. GRU del resto del
pipeline. Soporta feature_set="narrow" (Etapa 1, default) o "full"
(Etapa 2 — feature set completo de Track A ML). Ver
docs/decisions/track_b_etapa2_gru_full_features.md.

Uso:
    uv run python -m models.deep_learning.gru_runner
    uv run python -c "from models.deep_learning.gru_runner import main; main(feature_set='full')"
"""
...
def main(
    ...
    for h in horizons:
        ...
        logger.info(
            f"[gru_runner] Horizonte {h} (feature_set={feature_set}, "
        ...
        result = select_lookback(
            ...
            feature_cols,
            target_col,
            model_factory_for_input_size=lambda n: GRURegressor(
                input_size=n, hidden_size=HIDDEN_SIZE
            ),
        )
        logger.info(
            f"[gru_runner] h={h}: lookback elegido={result.lookback} "
        ...
        predictions_path = results_dir / f"gru_val_predictions_h{h}{suffix}.parquet"
        pl.DataFrame({"date": dates, "y_true": y_true, "y_pred": y_pred}).write_parquet(
            predictions_path
        )

        with mlflow.start_run(run_name=f"gru_h{h}{suffix}"):
            mlflow.log_params(
                {
                    "horizon": h,
    ...
    print(
        f"\nGRU — feature_set={feature_set}\n"
        f"{'Horizonte':<10}{'Lookback':<10}{'RMSE (GRU)':<14}"
        f"{'RMSE (naive)':<14}{'MAE (GRU)':<14}{'MAE (naive)':<14}"
    )
