"""
Entry point for Track A ML, Etapa 4 (Bloque 2 del plan de acción):
LightGBM por horizonte sobre el índice RFX20 — cierra el Bloque 2 - Track A.

Igual que Random Forest y XGBoost, LightGBM acepta NaN de forma nativa
(``use_missing=True`` por default), así que recibe las columnas de
futuros/TAMAR/MEP con sus nulos reales, sin excluirlas (ver
docs/decisions/ml_feature_engineering_track_a.md, Decisión 3).

Uso: uv run python -m models.ml.lightgbm_runner
"""
...
RESULTS_DIR = settings.RESULTS_DIR / "track_a_ml" / "lightgbm"
...
def main(
    ...
    for h in horizons:
        ...
        # n_jobs=1 here, not -1: RandomizedSearchCV already parallelizes across
        # fits (n_jobs=-1 in tune_and_evaluate). Letting LightGBM ALSO spawn
        # all-core threads per fit causes massive thread oversubscription —
        # measured ~2.5h/horizon instead of the expected seconds/minutes (see
        # docs/decisions/lightgbm_track_a.md). Parallelize at one level only.
    ...
    print(f"\n{'Horizonte':<10}{'RMSE (LGBM)':<14}{'RMSE (naive)':<14}{'MAE (LGBM)':<14}{'MAE (naive)':<14}")
